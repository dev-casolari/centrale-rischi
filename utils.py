...
def extract_text_segments(pdf_path):
  ...
  with pdfplumber.open(pdf_path) as pdf:
    ...
    for page_number in range(len(pdf.pages)):
      ...
      for _, segment in enumerate(text_segments[:-1]):
...
def extract_numbers_after_zero(text, section_name):
  ...
  matches = re.finditer(
      fr'{section_name}.*?0.*?(\d+(?:\.\d+)*(?:\.\d+)?)\D+(\d+(?:\.\d+)*(?:\.\d+)?)\D+(\d+(?:\.\d+)*(?:\.\d+)?)',
      text)
...
def extract_intermediario_names(text):
  ...
  intermediario_names = [match.group(1)
...
def extract_intermediario_section(text):
  ...
  matches = re.finditer(r'Intermediario:(.+?)(?=(Intermediario:|$))', text,
                        re.DOTALL)
...
def italian_date_to_datetime(italian_date):
  ...
  italian_date = italian_date.lower()
  for month, number in conversion_dict.items():
    if month in italian_date:
      italian_date = italian_date.replace(month, number)

  italian_date_format = "%m %Y"
  date_object = datetime.strptime(italian_date, italian_date_format)
...
def create_df_from_pdf(pdf_path):
  ...
  for example_text in resulting_text_segments:
    ...
    for i, section_text in enumerate(intermediario_sections):
      section_names = [
          "NAZIONALI DIVERSE DA", "RISCHI AUTOLIQUIDANTI", "RISCHI A SCADENZA",
          "RISCHI A REVOCA"
      ]
      ...
      for section_name in section_names:
        ...
        max_length = max(len(numbers_list),
                         1)  # 1 for the single Intermediario section

        for j in range(max_length):
          row_dict = {
              "Section Name": section_name,
              "Intermediario Name": intermediario_names[i]
          }

          if j < len(numbers_list):
            row_dict.update(
                {f"Number {k + 1}": numbers_list[j][k]
                 for k in range(3)})
          else:
            row_dict.update(
                {f"Number {k + 1}": float('nan')
                 for k in range(3)})
      ...
      dataframes_and_names_list.append({
          "Intermediario Name":
          intermediario_names[i],
          "DataFrame":
          df
      })
  ...
  new_column_names = [
      'Categoria', 'Intermediario', 'Accordato', 'Accordato Operativo',
      'Utilizzato', 'Periodo'
  ]
  ...
  df['Categoria'] = df['Categoria'].replace("NAZIONALI DIVERSE DA",
